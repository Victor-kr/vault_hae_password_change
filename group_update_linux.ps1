...
function createCredential {
    ...
    $body = "{`"name`":`"OS_${location}_linux`",`"active`":`"true`",`"credential_id`":`"servicenow/data/locations/${location}/linux`",`"context_name`":`"${context}`",`"use_high_security`":`"true`",`"credential_storage_vault`":`"none`"}"
    ...
    $response.Result
    Write-Host "=[Log : Update ServiceNow ]====== Success to create credential in ServiceNow : OS_${location}_linux"
}
...
$getCredentialUrl = "${servicenowUrl}/api/now/table/discovery_credentials?sysparm_query=nameSTARTSWITHOS_%5EnameENDSWITH_linux&sysparm_fields=name"
...
if(($notExistUnumberArray -ne $null) -and ($notExistUnumberArray.Count -ne 0)){
    ...
    $fileName = $today+"_Linux_Mismatch_between_Group_and_Cred"

    for($m=0;$m -lt $notExistUnumberArray.Count; $m++){
        $str = $notExistUnumberArray[$m]
        Add-Content C:\Vault\logs\group\${fileName}.txt "OS_${str}_linux"
    }
